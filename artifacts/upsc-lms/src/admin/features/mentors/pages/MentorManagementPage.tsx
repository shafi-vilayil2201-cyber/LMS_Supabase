...
import { getMentorApplications, getUsers, updateMentorApplicationStatus } from "@/shared/services/db";
...
export default function MentorManagementPage() {
  ...
  async function loadData() {
    ...
    // Map applicantName to name for compatibility
    const mappedApps = apps.map((a: any) => ({ ...a, name: a.applicantName || a.name }));
  ...
  async function handleApprove(id: string, name: string, applicantId?: string) {
    try {
      await updateMentorApplicationStatus(id, "approved", applicantId);
      toast({ title: "Mentor approved!", description: `${name} can now accept students.` });
      await loadData();
    } catch (err: any) {
      toast({ title: "Approval failed", description: err.message || "Database write error", variant: "destructive" });
    }
  }

  async function handleReject(id: string, name: string, applicantId?: string) {
    try {
      await updateMentorApplicationStatus(id, "rejected", applicantId);
      toast({ title: "Application rejected.", description: `${name}'s application has been declined.`, variant: "destructive" });
      await loadData();
    } catch (err: any) {
      toast({ title: "Rejection failed", description: err.message || "Database write error", variant: "destructive" });
    }
  }
              ...
              <div key={app.id} data-testid={`card-application-${app.id}`}
                ...
                <div className="flex items-start justify-between gap-4">
                  ...
                  <div className="flex gap-2 flex-shrink-0">
                    <button onClick={() => handleReject(app.id, app.name, app.applicantId)}
                    ...
                    <button onClick={() => handleApprove(app.id, app.name, app.applicantId)}
