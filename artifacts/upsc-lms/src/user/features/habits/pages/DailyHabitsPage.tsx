...
import { getDailyHabits, saveDailyHabits } from "@/shared/services/db";
...
export default function DailyHabitsPage() {
  ...
  async function handleSubmit() {
    try {
      ...
      await saveDailyHabits(currentUser.id, todayStr, todayChecked);
      toast({ title: "Habits saved!", description: `Today's discipline score: ${todayDisciplineScore.toFixed(1)} / 4` });
      
      // Reload daily habits history
      const h = await getDailyHabits();
      const userHabits = h.filter((x: any) => x.userId === currentUser.id).slice(0, 7);
      setHabits(userHabits);
    } catch (err: any) {
      toast({ title: "Failed to save habits", description: err.message || "Database write error", variant: "destructive" });
    }
  }
